  ...
  class CashFlow
    ...
    def card_selection(answer)
      if answer&.to_i.to_i <= @current_account.card.length && answer&.to_i.to_i.positive?
        { current_card: @current_account.card[answer&.to_i.to_i - 1], error: false }
      else
    ...
    def withdraw_money(current_card, a2, answer)
        ...
        if money_left.positive?
          ...
          { message: "Money #{a2&.to_i.to_i} withdrawed from #{current_card[:number]}$. Money left: #{current_card[:balance]}$. Tax: #{@tax.withdraw_tax(current_card[:type], current_card[:balance], current_card[:number], a2&.to_i.to_i)}$",
            return: true }
        else
...
#---------------------
    ...
    def put_money(current_card, a2, answer)
      if @tax.put_tax(current_card[:type], current_card[:balance], current_card[:number], a2&.to_i.to_i) >= a2&.to_i.to_i
        ...
        "Money #{a2&.to_i.to_i} was put on #{current_card[:number]}. Balance: #{current_card[:balance]}. Tax: #{@tax.put_tax(current_card[:type], current_card[:balance], current_card[:number], a2&.to_i.to_i)}"
      end
    end
...
#----------------
    ...
    def send_money(a3, a2, answer, sender_card, recipient_card, sender_balance, recipient_balance)
      if sender_balance.negative?
        ...
        result_info = <<~EOF 
        Money #{a3&.to_i.to_i}$ was withdrawn from #{sender_card[:number]}. Balance: #{sender_balance}. Tax: #{@tax.sender_tax(sender_card[:type], sender_card[:balance], sender_card[:number], a3&.to_i.to_i)}$
        Money #{a3&.to_i.to_i}$ was put on #{a2}. Balance: #{recipient_balance}. Tax: #{@tax.put_tax(sender_card[:type], sender_card[:balance], sender_card[:number], a3&.to_i.to_i)}$
        EOF
