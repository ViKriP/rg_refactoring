...
#----------------
    ...
    def send_money(a3, a2, answer, sender_card, recipient_card, sender_balance, recipient_balance)
      puts "#{@tax.put_tax(recipient_card[:type], recipient_card[:balance], recipient_card[:number], a3&.to_i.to_i)} >= #{a3&.to_i.to_i}"
      #puts "#{recipient_card[:type]} -- #{recipient_card[:balance]} -- #{recipient_card[:number]} -- #{a3&.to_i.to_i}) >= #{a3&.to_i.to_i}"
      if sender_balance.negative?
        ...
        @storage.save_data(new_accounts)
        result_info = <<~EOF 
        Money #{a3&.to_i.to_i}$ was put on #{sender_card[:number]}. Balance: #{recipient_balance}. Tax: #{@tax.put_tax(sender_card[:type], sender_card[:balance], sender_card[:number], a3&.to_i.to_i)}$\n
        Money #{a3&.to_i.to_i}$ was put on #{a2}. Balance: #{sender_balance}. Tax: #{@tax.sender_tax(sender_card[:type], sender_card[:balance], sender_card[:number], a3&.to_i.to_i)}$\n
        EOF
